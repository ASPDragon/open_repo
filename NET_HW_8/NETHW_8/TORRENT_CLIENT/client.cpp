...
int main(int argc, char const* argv[]) try
{
    ...
    if (std::strcmp(argv[1], "-f")) {
        lt::add_torrent_params file = lt::load_torrent_file(argv[2]);

        if (buf.size()) {
            lt::add_torrent_params ftp = lt::read_resume_data(buf);
            if (ftp.info_hashes == file.info_hashes) file = std::move(ftp);
        }

        file.save_path = ".";
        ses.async_add_torrent(std::move(file));
    }
    else if (std::strcmp(argv[1], "-m")) {
        // load resume data from disk and pass it in as we add the magnet link
        lt::add_torrent_params magnet = lt::parse_magnet_uri(argv[2]);
