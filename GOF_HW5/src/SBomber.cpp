...
SBomber::SBomber(bool _withChimney, bool _smallWindow, bool _isWinter)
        : exitFlag(false), startTime(0), finishTime(0), deltaTime(0), passedTime(0),
          fps(0), bombsNumber(10), score(0), withChimney{_withChimney}, smallWindow{_smallWindow}, isWinter{_isWinter} {
    ...
    if (withChimney)
        pHouse = director->BuildHouseChimney(*builder);
    else if (smallWindow)
        pHouse = director->BuildHouseSmallWindow(*builder);
    else
        pHouse = director->BuildHouseClassic(*builder);
    pHouse->SetWidth(13);
...
void SBomber::MoveObjects() {
    ...
    for (size_t i = 0; i < vecDynamicObj.size(); i++) {
        if (vecDynamicObj[i] != nullptr) {
            ...
            vecDynamicObj[i]->Accept(*visitor.get());
        }
    }
};
...
std::vector<DestroyableGroundObject*> SBomber::FindDestoyableGroundObjects() const {
    ...
    DestroyableGroundObject* pObj;
    for (auto& obj : vecStaticObj) {
        pObj = dynamic_cast<DestroyableGroundObject*>(obj);
        if (pObj != nullptr) {
            vec.push_back(pObj);
            continue;
        }
