...
void SBomber::CheckBombsAndGround() {
  ...
  auto plainIt = std::make_unique<BombIterator<DynamicObject, Bomb>>(vecDynamicObj);
  for (; plainIt->isValid(); plainIt->Next()) {
    if (plainIt->ObjectPtr()->GetY() >= y) {
      pGround->AddCrater(plainIt->ObjectPtr()->GetX());
      CheckDestoyableObjects(plainIt->ObjectPtr());
      DeleteDynamicObj(plainIt->ObjectPtr());
    }
  }
}
